...
#### Description:
     # Convert Raw microarray files profiled in hgu219 platforms into expression values!

################################################################################################################################################
###                                         Data Processing for Hgu219hs
################################################################################################################################################
  ...
  library(hgu219hsentrezg.db)

  Data <- ReadAffy(cdfname = "hgu219hsentrezgcdf")
  ...
  GS <- as.matrix(getSYMBOL(ID, 'hgu219hsentrezg.db'))    # Mapping the Probeset into the correspondent Gene Symbol
  EG <- as.matrix(getEG(ID, 'hgu219hsentrezg.db'))        # Mapping the Probeset into the correspondent EntrezGene
