...
#### Description:
     # Convert Raw microarray files profiled in Human Genome hgu133a platforms into expression values!

################################################################################################################################################
###                                            Data Processing for Hgu133a
################################################################################################################################################
  ...
  Data <- ReadAffy(cdfname = "hgu133ahsentrezgcdf")
  ...
  # Frma normalization!
  eset <- frma(Data, normalize="quantile", output.param = "hgu133ahsentrezgfrmavecs")
  ...
  GS <- as.matrix(getSYMBOL(ID, 'hgu133ahsentrezg.db'))    # Mapping the Probeset into the correspondent Gene Symbol
  EG <- as.matrix(getEG(ID, 'hgu133ahsentrezg.db'))        # Mapping the Probeset into the correspondent EntrezGene
